...
//			}
	...
	function openDoor(){///////随机打开门
		...
		function open(){
			...
			s=Math.floor(1+Math.random()*2)*500;
			se2=setTimeout(open,s);
			/*for(var i=0;i<feng.length;i++){
				feng[i].style.display="none";
			}*/
...
function page4(){
	...
	self.show=function(hdObj){
		...
		self.currentPage.style.display="block";
		var luckyNum = randomRange(0,20);
		if (luckyNum <= 6 || luckyNum >= 8) 
		{
			luckyNum = randomRange(0,3);
			...
			document.getElementById("p40Sec").innerHTML = (25-parseInt(LastS))+"S";
			document.title = '我只用'+(25-parseInt(LastS))+'秒就帮助大圣逃出天宫，超过'+Percent(LastS)+'%的小帮手，试试你能有多快？';
		}
		else if (luckyNum == 7) 
		{
			...
			document.getElementById("p47Sec").innerHTML = (25-parseInt(LastS))+"S";
			//我只用了10s中就帮助大圣逃出天宫，超过了90%的小帮手，还赢得了XX元话费，试试你能有多快？
			document.title = '我只用'+(25-parseInt(LastS))+'秒就帮助大圣逃出天宫，超过'+p+'%的小帮手，试试你能有多快？';
			$('#m_but471').hide();
			$('#m_but472').hide();
		};
...
function luckyDraw()
{
	...
	gift = gifts[randomRange(0, 0)];
	document.getElementById("p48Gift").innerHTML = gift; 
	//我只用了10s中就帮助大圣逃出天宫，超过了90%的小帮手，还赢得了XX元话费，试试你能有多快？
	document.title = '我只用'+ (25-parseInt(LastS))+'秒就帮助大圣逃出天宫，超过'+Percent(LastS)+'%的小帮手，还赢得了'+gift+',试试你能有多快？';
}
