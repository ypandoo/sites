...
//Update week honer
$sql = "UPDATE `honer_week` SET `prized` = 1 WHERE `start_date` = '".$start_date."' and `end_date` = '".$end_date."' and `month` = '".$month."' and `phone` = '".$phone."'";
$mysql->runSql($sql);
if($mysql->errno() != 0)
{
 $arr = array('status'=>'0','msg'=>"更新周冠信息失败".$sql);
 $mysql->closeDB();
