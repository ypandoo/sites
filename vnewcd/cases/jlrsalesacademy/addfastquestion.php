...
$link = conn_db();
// inset the question
$sql="INSERT INTO  `fastquestion`  (`que_name`, `que_answer`, `que_score`, `que_date`, `department`) VALUES ('$q_name', '$answer', '20', '$q_date', '$department')";
$ret=runquery($sql,$link);

// insert or update the options for the questions
$qsql="SELECT * FROM `fastquestion` where que_name='$q_name' and que_date='$q_date'";
$qids=getaline($qsql,$link);

// update the options
$qid=$qids['qid'];
$sql="replace INTO `fastoptions` (`qid`, `A`, `B`, `C`, `D`) VALUES ('$qid', '$opt_a', '$opt_b', '$opt_c', '$opt_d')";
runquery($sql,$link);
