...
class Content_model extends CI_Model
{
    ...
    public function  addItem($dataArry){
        ...
        $this->db->insert('T_ZCSY', $dataArry);
    ...
    public function  updateContent($dataArry){
        $this->db->replace('T_ZCSY', $dataArry);
    ...
    public function getList($list_type, $page_start)
    {
        ...
        $query = $this->db->get_where('T_ZCSY', array('CONTENT_TYPE' => $list_type), $interval, $page_start);
        $data["success"] = true;
        $data["errorCode"] = 0;
        $data["error"] = 0;
    ...
    {
        $this->db->delete('T_ZCSY', array('CONTENT_ID' => $content_id));
        $data["success"] = true;
        $data["errorCode"] = 0;
        $data["error"] = 0;
    ...
    {
        $query = $this->db->get_where('T_ZCSY', array('CONTENT_ID' => $content_id));
        $data["success"] = true;
        $data["errorCode"] = 0;
        $data["error"] = 0;
